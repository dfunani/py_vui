...
class NodeGraphicsItem(QGraphicsRectItem):
    def __init__(self, node_id: str, label: str, w: float, h: float) -> None:
...
class DesignCanvas(QGraphicsView):
    ...
    def rebuild(self) -> None:
        ...
        root_item = self._items[root.id]
        if isinstance(root, WindowNode):
            rw, rh = root.props.width, root.props.height
            root_item.setRect(0, 0, rw, rh)
    ...
    def _add_node_recursive(self, node: Node) -> None:
        ...
        item = NodeGraphicsItem(node.id, label, box.w, box.h)
        item.setPos(abs_pos)
    ...
    def dropEvent(self, event) -> None:
        ...
        rel = self._relative_pos(parent_id, scene_pos)
    ...
    def _parent_at(self, scene_pos: QPointF) -> str | None:
        if self._doc is None:
            return None
        hits = self._scene.items(scene_pos)
        for hit in hits:
    ...
    def _relative_pos(self, parent_id: str, scene_pos: QPointF) -> QPointF:
        parent_abs = self._abs_pos.get(parent_id, QPointF(0, 0))
        return scene_pos - parent_abs
    ...
    def mouseReleaseEvent(self, event) -> None:
        ...
        parent_abs = self._abs_pos.get(node.parent_id or "", QPointF(0, 0))
        new_x = item.pos().x() - parent_abs.x()
        new_y = item.pos().y() - parent_abs.y()
        box = node.layout.box
